**Simple API [Guide introducción a Node]**
---
...
Ejecutar comando de creación de fichero **`.json`**
...
**`package.json`** → Fichero de gestor de paquetes que trabaja con npm.

**express →** Es una librería que nos ayuda a crear una Api Rest: Gestionar métodos propios de la librería.
...
> *dependencies : son dependencias o librerías que necesita nuestro proyecto.
> 

**mongoose** → Es una librería que me permite interactuar y conectarme a DB.
...
**cors** → Librería para gestión de proxies | “URL” permitidas.
...
**body-parser** → Librería que nos ayuda a transformar la data.
...
> *devdependencies : son dependencias o librerías que necesita nuestro proyecto en el desarrollo pero no en la puesta a producción.
> 
...
Añadimos los campos necesarios como autor, licencia, descripción... pero lo más importante son los scripts de ejecución de **`npm`**.
...
Si cogéis otro **`package.json`** o este de referencia podéis lanzar un **`npm i`** en vuestra terminal y os instala todos los paquetes según el fichero de configuración.

**Estructura de carpetas**
---

Dentro de server me creo una carpeta **`src`** [source]→ hace referenci al código de mi aplicación → Dentro genero dos carpetas **`api`** [endpoints] y **`utils`** [utilidades transversales].
...
El **`.env`** es el fichero de variables de entorno o configuración - lo gestionamos con el paquete dotenv. Lo añadimos a nivel de la carpeta raíz [**`server`**].
...
Es un fichero que indica a git → repositorio → qué subir y qué no subir. Lo añadimos a nivel de la carpeta raíz [**`server`**].
...
Dentro del fichero **`.gitignore`** debemos añadir el siguiente contenido.
...
Una carpeta dentro de **`utils`** → **`database`** → un fichero **`db.js`**
...
**Configuración de nuestro fichero de entrada → `index.js`**
---

Lo primero es crear nuestro fichero **`index.js`** en la carpeta raíz [**`server`**]
...
Dentro de la carpeta **`api`** → **`actors`** | **`movies`**. Y en cada una de ellas vamos a crear los ficheros sobre los que trabajaremos.
...
Un controlador es un conjunto de funciones | acciones que solicitan, envían, eliminan o modifican la info de la DB.
...
**`actors.routes.js`** endpoints asociados a la función del controlador.
...
**`movie.routes.js`** endpoints asociados a la función del controlador.
...
Os recuerdo que el fichero principal que dispara toda nuestra aplicación es el **`index.js`** y por ello debemos importar nuestras rutas o endPoints.
...
Lo primero nos descargamos la versión gratuita de esta URL:
...
Lo primero que vamos a hacer es eliminar la dependencia de **`body-parser`** porque viene configurado en el paquete de express. En el fichero **`package.json`** → eliminamos **`body-parser.`**
...
Información de nuestra API → dentro de la carpeta **`utils`** → generamos otra carpeta → `documentation` → dentro el fichero **`api.json`**
...
en nuestro **`index.js`** añadimos la info.
...
Y el resultado sería cuando vayamos a [http://localhost:8000/api](http://localhost:8000/api)